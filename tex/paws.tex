...
\addto\captionsspanish{\renewcommand{\tablename}{Tabla}}  % Para que no la llame Cuadro
\usepackage[colorlinks=true,citecolor=black,linkcolor=black,urlcolor=black]{hyperref}
\usepackage[nottoc,numbib]{tocbibind}  % Para que aparezca la bibliografía en el índice
\usepackage{array}      % Para opciones de tabla
\usepackage{caption}    % Para poner margen tras el caption de la tabla
...
\newcommand{\footnoteref}[2]{\hyperref[#1]{\textsuperscript{#2}}}
...
\newcolumntype{L}[1]{>{\raggedright\arraybackslash}p{#1}}  % Columna de ancho determinado alineada a la izquierda


\title{Guía unificada de los sistemas de creación de aventuras conversacionales
       \\\ \\\Huge{\quill, \paw, \swan y \daad}}
\author{\copyright\ 2010, 2018-2025 José Manuel Ferrer Ortiz
       \\\ \copyright\ 2023 José Luis Cebrián}

\hypersetup{pdftitle={Guía unificada de los sistemas de creación de aventuras conversacionales Quill, PAW, SWAN y DAAD}, pdfauthor={José Manuel Ferrer Ortiz y José Luis Cebrián}}
...
\hline
\textbf{Rango} & \textbf{Sistema} & \textbf{Tipo} & \textbf{Función} \\ \hline
0         & Todos                   & Todos          & Reservado para marcar fin de secciones en la base de datos  \\ \hline
1         & Con editor por menús    & Todos          & Reservado para la palabra comodín asterisco (\texttt{*})    \\ \hline
1 a 12    & \quill                  &                & Verbo de dirección                                          \\ \hline
1 a 13    & Compilados              & Nombre o verbo & Verbo de dirección                                          \\ \hline
2 a 13    & \paw por menús          & Nombre o verbo & Verbo de dirección                                          \\ \hline
1 a 19    & Todos                   & Nombre         & Convertible a verbo                                         \\ \hline
1 a 19    & \swan                   & Prepo-sición   & Convertible a verbo                                         \\ \hline
1 a 39    & \daad\newline versión 2 & Nombre         & Convertible a verbo                                         \\ \hline
1 a 49    & \paw y \daad            & Nombre         & Nombre propio (no referible con pronombres)                 \\ \hline
200 a 254 & \quill en Sinclair QL   &                & Aplicado sobre objeto, se podrá vestir y desvestir          \\ \hline
255       & Todos                   & Todos          & Reservado para la palabra comodín (normalmente \texttt{\_}) \\ \hline
\end{tabular}
\end{table}
...
\quill proporciona 33 de estas banderas (numeradas de 0 a 32), salvo en la plataforma Sinclair QL, donde proporciona 63 banderas (de la 0 a la 62 \cite{QuillQL}).

Fuera de ese rango hay algunas banderas más, llamadas banderas de sistema, que contienen información adicional necesaria para ejecutar la aventura: como la localidad actual, el número máximo de objetos llevables, o las palabras de la sentencia lógica actual.

Los editores y compiladores originales de \quill comercializados para el público general no permiten que se acceda ni se modifique a las banderas de sistema. No obstante, los intérpretes sí suelen permitirlo sin trabas, por lo que con herramientas como NAPS\cite{NAPS} se puede construir o adaptar aventuras que accedan a las banderas de sistema para funcionalidad avanzada.

Los sistemas sucesores a \quill (\paw, \swan y \daad) proporcionan 256 banderas (que van desde la 0 hasta la 255), que incluyen tanto las banderas accesibles ordinariamente desde \quill (también conocidas como banderas de usuario), como las banderas de sistema.

Es importante tener en cuenta que no todas las banderas están disponibles para cualquier propósito, ya que algunas de ellas tienen un significado predeterminado, y nos permitirán intercambiar con el intérprete información importante para la aventura; y otras son auto-decrementadas o auto-incrementadas por éste durante la ejecución de la aventura.
...
Esta bandera no tiene ningún propósito predefinido y, por tanto, puede ser utilizada libremente por el programador.
\\\ \\
\sistema{\quill al menos en CP/M\cite{QuillQDB}}
\newcounter{sysflagCPM}
Esta bandera es de sistema, normalmente no accesible por métodos usuales, pero el intérprete no pone impedimento para usarla, por lo que se puede utilizar con herramientas que permitan su uso\footnote{\ Como NAPS\cite{NAPS}, o txt2qdb para CP/M, que forma parte de UnQuill\cite{UnQuill} desde su versión 0.15\label{sysflagCPM}}\setcounter{sysflagCPM}{\value{footnote}}.

Contiene la primera palabra de la sentencia lógica actual.
\\\ \\
...
Esta bandera es de sistema, normalmente no accesible por métodos usuales, pero el intérprete no pone impedimento para usarla, por lo que se puede utilizar con herramientas que permitan su uso\footnoteref{sysflagCPM}{\thesysflagCPM}.

Contiene la segunda palabra de la sentencia lógica actual.
\\\ \\
\sistemas{\paw, \swan y \daad}
...
Contiene la tercera palabra de la sentencia lógica actual.
\\\ \\
\sistemas{\paw, \swan y \daad}
...
Contiene la cuarta palabra de la sentencia lógica actual.
\\\ \\
\sistemas{\paw, \swan y \daad}
...
Esta bandera no tiene ningún propósito predefinido y, por tanto, puede ser utilizada libremente por el programador.
\\\ \\
\sistemas{\quill al menos en CP/M\cite{QuillQDB}, \paw, \swan y \daad}

En \quill, esta bandera es de sistema, normalmente no accesible por métodos usuales, pero el intérprete no pone impedimento para usarla, por lo que se puede utilizar con herramientas que permitan su uso\footnoteref{sysflagCPM}{\thesysflagCPM}.

Almacena el número máximo de objetos que el jugador es capaz de llevar (en su inventario) a la vez. Su valor inicial es 4, y en los sistemas posteriores a \quill puede ser modificado con el condacto ABILITY.
...
Esta bandera no es solo para fines informativos, sino que modificarla afectará al intérprete, el cual usa la posición de memoria de esta bandera para todo lo que le requiera consultar en qué localidad está el jugador.
...
Esta bandera contiene la parte más significativa (MSB) del número de turnos que han transcurrido desde el inicio del juego. Ver explicación en la \hyperref[flag62]{\textbf{bandera 62}}.
\\\ \\
\sistema{\paw}
...
\subsection{Bandera 62}\label{flag62}
...
Es decir, que cuando hayan pasado 255 turnos, esta bandera tendrá un valor de 255 y la 61 un valor de 0; y cuando se pase a 256 turnos, esta bandera guardará un 0 y la 61 un 1. Dicho de otra forma aún, el número total de turnos será 256 multiplicado por el valor de la bandera 61, más el valor de esta bandera.
\\\ \\
\sistema{\paw}
...
Esta bandera es de sistema, no accesible por métodos usuales, pero el intérprete no pone impedimento para usarla, por lo que se puede utilizar con herramientas que permitan su uso, como NAPS\cite{NAPS}.

Contiene el número de localidad actual del jugador, equivalente a la bandera 38 en los sistemas sucesores de \quill.
\\\ \\
\sistema{\paw}
...
Este valor se actualiza por el condacto WINDOW y su propósito es informativo. Es decir, modificarlo no tiene ningún efecto en el intérprete.

\subsection{Banderas 64 a 255}

\sistema{\quill en QL}

Estas banderas son de sistema, no accesibles por métodos usuales, pero el intérprete no pone impedimento para usarlas, por lo que se pueden utilizar con herramientas que permitan su uso, como NAPS\cite{NAPS}.

La bandera 64 contiene la localidad actual del objeto 0. En caso de tener la aventura más de un objeto, las banderas posteriores a la 64 contienen la localidad de los objetos siguientes, conteniendo por ejemplo la bandera 65 la localidad correspondiente al objeto 1, la 66 la del objeto 2, etc. Y la bandera siguiente al número de objetos (es decir, la bandera 65 en aventuras que solo tengan un objeto), contiene un valor 255, indicando el fin de la lista de ubicación de los objetos, que no se debería modificar para no entorpecer el correcto funcionamiento del intérprete.

Las banderas siguientes (posteriores a la que indica el fin de la lista de ubicación de los objetos) hasta la bandera 255 inclusive, pueden utilizarse sin problema como banderas de propósito general. No obstante, estas banderas no se inicializarán, por lo que conservarán su valor incluso si se reinicia la aventura, por lo que se deberán inicializar manualmente. Pero su valor sí se almacenará tanto al guardar la partida con el condacto SAVE, como con el condacto RAMSAVE, y se recuperará su valor con las acciones LOAD y RAMLOAD.
\\\ \\
\sistemas{\paw y \daad}
...
Los mensajes de sistema son mensajes de información que se le ofrecen al jugador, en distintos contextos. A diferencia de los mensajes de usuario, algunos de estos mensajes son mostrados automáticamente por el intérprete, aunque las aventuras de \paw y \daad pueden mostrarlos también en cualquier momento, haciendo uso de la acción SYSMESS (no existente en \quill salvo en la plataforma Sinclair QL\cite[pág. 5]{QuillAWRef}).

\quill en casi todas las plataformas tiene 32 mensajes de sistema, todos ellos con funcionalidades determinadas de antemano. Pero \paw y \quill para QL permiten hasta 256 mensajes de sistema, de los cuales menos de una cuarta parte tiene finalidades predeterminadas, pudiendo los demás utilizarse libremente. Esto, junto con los mensajes de usuario, deja al programador de aventuras de \paw o \quill para QL con más de 440 mensajes de propósito general, mientras que \quill en la mayoría de plataformas solo proporciona los 256 mensajes de usuario.

El sistema \daad provee en su versión 1 el mismo número total de mensajes de propósito general (todos los de usuario y los libres de sistema). Pero en su versión 2, la cifra de mensajes disponibles de propósito general aumenta fácilmente hasta los 600, dado que el condacto DESC en esta versión se comporta de modo similar a las acciones MESSAGE y SYSMESS, permitiendo imprimir los mensajes de descripciones de localidad, por lo que se pueden aprovechar aquellos no utilizados por las localidades de la aventura.

Como anécdota, Javier San José, uno de los ganadores del concurso de aventuras de MicroHobby que recibió una copia del sistema DAAD, en un mensaje \cite{JSJ} de la lista de correo del CAAD, hablaba del soporte de ¡hasta 256 tablas de mensajes! (supongo, contando la de sistema):
...
\begin{description}
  ...
  \item[Tamaño de la ventana] Por defecto: [25, 53] en modo gráfico y [25, 80] en modo texto (véase la información de la \textbf{\nameref{flag29}}). Se cambia a través del condacto WINSIZE.
...
\bibitem{QuillQDB}
  John Elliott,
  ``\emph{UnQuill: Disassemble games written with the `Quill' adventure game system}''.\\
  \url{https://www.seasip.info/Unix/UnQuill}

\bibitem{UnQuill}
  John Elliott,
  ``\emph{Quill QDB file format}''.
  Version 0.2,
  15 de septiembre de 2023.\\
  \url{https://www.seasip.info/Unix/UnQuill/qdb.html}
...
\bibitem{NAPS}
  ...
  ``\emph{NAPS - The New Age PAW-like System - Herramientas para sistemas PAW-like}''.\\
  \url{https://github.com/daad-adventure-writer/NAPS}
